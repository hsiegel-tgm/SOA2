...
An code example for WSDL can be found in section \ref{sec:wsdlexaple}, 
and a code example for SOAP can be found in section \ref{sec:soapexaple}
\subsection{JSON}
...
\centering
    ...
    \caption{Normal Software Development cycle \cite{TODO}}
    \label{fig:asdas}
\end{minipage}
...
\end{lstlisting}
As it can be that JSON is easier to understand than XML.\cite{jsonxml}
\subsection{SOAP Example}
\label{sec:soapelexaple}
In the example below, a GetStockPrice request is sent to a server. The request has a StockName parameter, and a Price parameter that will be returned in the response. The namespace for the function is defined in \texttt{http://www.example.org/stock}.\\
SOAP request:
\begin{lstlisting}
